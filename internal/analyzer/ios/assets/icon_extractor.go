...
let iconName = CommandLine.arguments[2]

guard let bundle = Bundle(path: bundlePath) else { exit(1) }
guard let image = bundle.image(forResource: iconName) else { exit(1) }
...
func ExtractIconFromCar(carPath string, iconNames []string, catalogAssets []AssetInfo) ([]byte, error) {
	...
	for _, name := range candidates {
		data, err := extractIconByName(carPath, name)
		if err == nil && len(data) > 0 {
			return data, nil
		}
	}

	return nil, fmt.Errorf("no icon found in Assets.car (tried %d names)", len(candidates))
}

// extractIconByName extracts a named icon from an Assets.car file by creating
// a temporary bundle structure and running a Swift script to load it via NSBundle.
func extractIconByName(carPath string, iconName string) ([]byte, error) {
	...
	// Run: swift <script> <bundle-path> <icon-name>
	cmd := exec.Command("swift", scriptFile.Name(), bundleDir, iconName)
	var stdout, stderr bytes.Buffer
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr

	if err := cmd.Run(); err != nil {
		return nil, fmt.Errorf("swift icon extraction failed for %q: %w", iconName, err)
	}
	...
	if len(data) < len(pngSignature) || !bytes.Equal(data[:len(pngSignature)], pngSignature) {
		return nil, fmt.Errorf("output is not a valid PNG for icon %q", iconName)
	}
